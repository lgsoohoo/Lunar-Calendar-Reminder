...
let startYear = 2019; // Year to start generating event reminders
let yearsToGenerate = 5; // Number of years to generate
...
if (startYear < 1900 || (startYear + yearsToGenerate) > 2100) {
    console.log(`Error: Program is limited to years 1900 to 2100.\nEntered years: ${startYear} to ${startYear+yearsToGenerate}.`);
} else {

    console.log(`Program is running. Please wait ${yearsToGenerate} seconds.`)
    ...
    for (let year = startYear; year < startYear + yearsToGenerate; year++) {
        ...
        // schedule requests after [0, 0.5, 1, 1.5, 2, ...] seconds
        // Each request should be roughly 0.5 seconds after the previous
    }

    setTimeout(function() {
        // Print CSV file data after all HTTP requests have (probably) finished
        console.log(csvFile)
    }, yearsToGenerate * 1000);
