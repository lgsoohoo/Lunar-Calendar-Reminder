/*
    ...
    Lunar Calendar Reminder (Proof of Concept version)
    Lucas Soohoo, August 2019

    This is the original Proof of Concept version of the program.
    This version is (spaghetti code and is) not recommended. A revised version is
    available in the /website folder and is available online at <site coming soon>.
    ...
    This Proof of Concept version can be run in your browser and will print out the data
...
let endYear = 2020; // Year to stop generating event reminders (inclusive)
...
function lunarToGregorian(varYear, varMonth, varDay) {
    ...
    let convertRequest = new XMLHttpRequest();

    let link =
        "https://pteo.paranoiaworks.mobi/chinese_calendar/cncalprov.php?g2c=0&year=" +
        varYear +
        "&month=" +
        varMonth +
        "&date=" +
    ...
    convertRequest.onreadystatechange = function() {
        if (convertRequest.readyState == XMLHttpRequest.DONE) {
            if (convertRequest.status == 500) {
                // Received an error. Server is down (probably) because we sent too many requests too quickly
                // We need to resubmit
                console.log(
                    `Received 500 server error for year: ${varYear}. Resubmitting...`
                );
                setTimeout(function() {
                    // Add a small delay to allow server to recover
                    convertRequest.open("GET", link);
                    convertRequest.send();
                }, httpDelayLength);
            } else if (convertRequest.status == 0) {
                ...
                if (rawRes.includes("Invalid")) {
                    console.log(
                        `${varMonth}/${varDay} is an invalid date for ${varYear}. Skipping...`
                    );
                } else {
    ...
    }; // end convertRequest.onreadystatechange = function()...
...
if (startYear < 1900 || endYear > 2100) {
    console.log(
        `Error: Program is limited to years 1900 to 2100.\nEntered years: ${startYear} to ${endYear}.`
    );
    ...
    console.log(
        `Program is running. Please wait ${Math.round(
      printCSVDelayLength / 1000
    )} seconds.`
    );
